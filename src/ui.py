...
class App(ctk.CTk):
    def __init__(self):
        ...
        try:
            # Assuming app_icon.ico is in the same directory as ui.py
        ...
        if not PANDOC_INSTALLED: 
            self.pandoc_warning_label = ctk.CTkLabel(self, 
        ...
        self.cb_embed_small_images = ctk.CTkCheckBox(self.config_frame, text="Embed small images as base64 (PDFs only)")
        self.cb_embed_small_images.grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.small_image_kb_label = ctk.CTkLabel(self.config_frame, text="Small image threshold (KB):")
        self.small_image_kb_label.grid(row=0, column=1, padx=5, pady=5, sticky="e")
        self.small_image_kb_entry = ctk.CTkEntry(self.config_frame, width=50)
        self.small_image_kb_entry.grid(row=0, column=2, padx=5, pady=5, sticky="w")
        self.small_image_kb_entry.insert(0, "5")
        
        self.cb_exclude_decorative = ctk.CTkCheckBox(self.config_frame, text="Exclude decorative images (PDFs only)")
        self.cb_exclude_decorative.grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.decorative_px_label = ctk.CTkLabel(self.config_frame, text="Decorative threshold (pixels, W & H <):")
        self.decorative_px_label.grid(row=1, column=1, padx=5, pady=5, sticky="e")
        self.decorative_px_entry = ctk.CTkEntry(self.config_frame, width=50)
        self.decorative_px_entry.grid(row=1, column=2, padx=5, pady=5, sticky="w")
        self.decorative_px_entry.insert(0, "50")
        
        self.cb_apply_max_res = ctk.CTkCheckBox(self.config_frame, text="Apply max image resolution (PDFs only)")
        self.cb_apply_max_res.grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.max_res_px_label = ctk.CTkLabel(self.config_frame, text="Max image resolution (pixels, W/H):")
        self.max_res_px_label.grid(row=2, column=1, padx=5, pady=5, sticky="e")
        self.max_res_px_entry = ctk.CTkEntry(self.config_frame, width=50)
        self.max_res_px_entry.grid(row=2, column=2, padx=5, pady=5, sticky="w")
        self.max_res_px_entry.insert(0, "1024")
        
        # Add timeout option for external images
        self.cb_limit_image_download = ctk.CTkCheckBox(self.config_frame, text="Set timeout for external images")
        self.cb_limit_image_download.grid(row=3, column=0, padx=5, pady=5, sticky="w")
        self.cb_limit_image_download.select()  # Checked by default
        self.image_timeout_label = ctk.CTkLabel(self.config_frame, text="Download timeout (seconds):")
        self.image_timeout_label.grid(row=3, column=1, padx=5, pady=5, sticky="e")
        self.image_timeout_entry = ctk.CTkEntry(self.config_frame, width=50)
        self.image_timeout_entry.grid(row=3, column=2, padx=5, pady=5, sticky="w")
        self.image_timeout_entry.insert(0, "90")  # 90 seconds default timeout

        self.input_folder_path_label = ctk.CTkLabel(self, text="Input Folder Path (containing source files):")
        self.input_folder_path_label.pack(pady=(10,0), padx=10, anchor="w")
        self.input_folder_path_frame = ctk.CTkFrame(self)
        self.input_folder_path_frame.pack(fill="x", padx=10, pady=(0,5))
        self.input_folder_path_entry = ctk.CTkEntry(self.input_folder_path_frame, placeholder_text="Select folder with source files...")
        self.input_folder_path_entry.pack(side="left", fill="x", expand=True, padx=(0,5))
        self.select_input_folder_button = ctk.CTkButton(self.input_folder_path_frame, text="Select Input Folder", command=self.select_input_folder)
        self.select_input_folder_button.pack(side="left")

        self.output_folder_path_label = ctk.CTkLabel(self, text="Output Folder Path (ZIP archives will be saved here):")
        self.output_folder_path_label.pack(pady=(10,0), padx=10, anchor="w")
        self.output_folder_path_frame = ctk.CTkFrame(self)
        self.output_folder_path_frame.pack(fill="x", padx=10, pady=(0,5))
        self.output_folder_path_entry = ctk.CTkEntry(self.output_folder_path_frame, placeholder_text="Select folder for outputs (or leave blank for default)...")
        self.output_folder_path_entry.pack(side="left", fill="x", expand=True, padx=(0,5))
        self.select_output_folder_button = ctk.CTkButton(self.output_folder_path_frame, text="Select Output Folder", command=self.select_output_folder)
        self.select_output_folder_button.pack(side="left")

        self.file_name_label = ctk.CTkLabel(self, text="Output File Suffix (optional, appends to original name):")
        self.file_name_label.pack(pady=(5,0), padx=10, anchor="w")
        self.file_name_entry = ctk.CTkEntry(self, placeholder_text="e.g., 'rag_version'")
        self.file_name_entry.pack(fill="x", padx=10, pady=(0,10))

        self.convert_button = ctk.CTkButton(self, text="Convert", command=self.start_conversion_thread)
    ...
    def update_status(self, message):
        # Check if message contains external image notification
        if "external image(s)" in message:
            self.status_label.configure(text=message, text_color="orange")
        else:
            self.status_label.configure(text=message, text_color=("gray10", "gray90"))  # Reset to default color
    ...
    def run_conversion(self, input_folder, output_base_folder, output_suffix):
        try:
